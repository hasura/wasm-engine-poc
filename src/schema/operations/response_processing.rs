//! Process the response from the GDC client to be a GraphQL response.
...
use crate::schema::types::{global_id_col_format, OutputAnnotation};
use crate::schema::{
    types::{Annotation, GlobalID, GLOBAL_ID_VERSION},
    GDS,
};
    ...
    for field_name in global_id_fields {
        ...
        let field_json_value_result =
            row.remove(global_id_ndc_field_name.as_str())
                .ok_or_else(|| InternalDeveloperError::BadGDCResponse {
...
) -> Result<IndexMap<ast::Alias, json::Value>, Error>
where
    T: KeyValueResponse,
{
                        ...
                        match field_annotation {
                            ...
                            OutputAnnotation::RelayNodeInterfaceID { typename_mappings } => {
                                let global_id_fields = typename_mappings.get(type_name).ok_or(
                                    InternalDeveloperError::GlobalIDTypenameMappingNotFound {
                            ...
                            OutputAnnotation::RelationshipToModel { .. } => {
                                ...
                                match relationship_json_value_result {
                                    None => Err(InternalDeveloperError::BadGDCResponse {
...
pub fn process_selection_set_as_list(
    row_set: RowSet,
    selection_set: &normalized_ast::SelectionSet<'_, GDS>,
) -> Result<Option<Vec<IndexMap<ast::Alias, json::Value>>>, Error> {
...
pub fn process_selection_set_as_object(
    row_set: RowSet,
    selection_set: &normalized_ast::SelectionSet<'_, GDS>,
) -> Result<Option<IndexMap<ast::Alias, json::Value>>, Error> {
    let processed_response = row_set
        .rows
...
pub fn process_command_rows(
    ...
    match rows {
        ...
        Some(row_vector) => {
            if row_vector.len() > 1 {
                Err(InternalDeveloperError::BadGDCResponse {
                    summary: format!(
...
fn process_command_response_row(
    ...
    if selection_set.fields.is_empty() {
        ...
        match field_value_result.0 {
            ...
            json::Value::Array(values) => {
                if type_container.is_list() {
                    ...
                    let r: Vec<IndexMap<Alias, json::Value>> = array_values
                        ...
                        .collect::<Result<Vec<IndexMap<ast::Alias, json::Value>>, Error>>()?;
            ...
            _ => Err(InternalDeveloperError::BadGDCResponse {
                summary:
