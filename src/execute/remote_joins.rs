use crate::schema::operations::remote_joins::JoinLocations;
use crate::{
    schema::operations::{
        remote_joins::{
            ArgumentId, Arguments, JoinId, Location, MonotonicCounter, RemoteJoin,
            ReplacementToken, ReplacementTokenRows,
        },
        InternalEngineError,
    },
    utils::json_ext::ValueExt,
};
...
use super::operation::execute_ndc_query;
...
fn collect_arguments<'s>(
...
) -> Result<Option<CollectArgumentResult<'s>>, operations::Error> {
    ...
    match remote_join {
        None => Err(operations::Error::from(
...
fn collect_argument_from_row<'s>(
    ...
    if location.join_node.is_none() && location.rest.locations.is_empty() {
        return Err(operations::Error::from(
            InternalEngineError::InternalGeneric {
    ...
    if let Some((join_node, join_id)) = &location.join_node {
        ...
        *remote_alias = key.to_string();
        ...
        let nested_val = row.get(key).ok_or(InternalEngineError::InternalGeneric {
...
fn resolve_command_response_row(
...
) -> Result<Vec<IndexMap<String, ndc::models::RowFieldValue>>, operations::Error> {
    let field_value_result = row.get(String::from("__value").as_str()).ok_or_else(|| {
        operations::InternalDeveloperError::BadGDCResponse {
    ...
    match &field_value_result.0 {
        json::Value::String(_) | json::Value::Bool(_) | json::Value::Number(_) => {
            Err(operations::InternalDeveloperError::BadGDCResponse {
...
fn traverse_path_and_insert_value(
    ...
    if let Some(_join_node) = &location.join_node {
        ...
        let mut rows = row_set.rows.ok_or(InternalEngineError::InternalGeneric {
...
mod tests {
    ...
    use gds::schema::operations::remote_joins::{JoinLocations, Location, RemoteJoin};
